...
async function main() {
  ...
  try {
    ...
    const depositFeed = priceFeeds.find(
      (f: any) => normalizeCoinType(f.coinType) === normalizeCoinType(depositPool.coinType)
    );
    const usdcFeed = priceFeeds.find(
      (f: any) => normalizeCoinType(f.coinType) === normalizeCoinType(usdcPool.coinType)
    );
    ...
    console.log(`  Expected from swap: ${formatUnits(expectedOutput, decimals)} ${symbol}`);
    console.log(`  Total to deposit:   ${formatUnits(totalDepositAmount, decimals)} ${symbol}`);
    ...
    if (isSui) {
      ...
      console.log(`  Step 3: Split user's SUI from gas and merge with swapped SUI`);
    ...
    console.log(`  Step 5: Deposit ${formatUnits(totalDepositAmount, decimals)} ${symbol} to Navi`);
    ...
    console.log(`  Step 6: Borrow ${formatUnits(repaymentAmount, 6)} USDC from Navi`);
    const borrowedUsdc = await borrowCoinPTB(tx as any, usdcPool, Number(repaymentAmount), {
      env: 'prod',
    });
