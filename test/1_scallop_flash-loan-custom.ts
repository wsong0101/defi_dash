...
async function testFlashLoanWithCustomClient() {
  ...
  const FLASH_LOAN_COIN_TYPE = process.env.FLASH_LOAN_COIN_TYPE || COIN_TYPES.USDC;
  ...
  console.log(
    `  Amount:      ${formatUnits(loanAmount, decimals)} ${symbol} (Raw: ${loanAmount.toString()})`
  );
  ...
  try {
    ...
    const [loanCoin, receipt] = flashLoanClient.borrowFlashLoan(tx, loanAmount, coinName);
    ...
    console.log(`💰 Repaying ${formatUnits(loanAmount, decimals)} ${symbol}...`);
